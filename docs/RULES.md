...
If executable prose requests an external action (`must use the internet`, `call an API`, or `send email`), the body of `Approval Requirements` must substantively require approval or consent. The section heading alone does not satisfy this rule. External-action phrases inside fenced or indented code examples are ignored.
